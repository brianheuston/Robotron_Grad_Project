...
LRESULT GameEngine::HandleEvent(HWND hWindow, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (msg)
  {
    ...
    case MM_MCINOTIFY:
      // See if MIDI has finished playing
      if (wParam != MCI_NOTIFY_FAILURE){
        if (m_msMIDIState == MIDI_PLAYING && wParam != MCI_NOTIFY_ABORTED)
          m_msMIDIState = MIDI_DONE;
        MIDINotifying(m_msMIDIState);
      }
      else
...
void GameEngine::PlayMIDISong(LPSTR szMIDIFileName, BOOL bRestart)
{
    ...
    if(mciSendCommand(m_uiMIDIPlayerID, MCI_PLAY, MCI_NOTIFY,
        (DWORD_PTR)&mciPlayParms) != 0)
            // There was a problem, so close the MIDI player
            CloseMIDIPlayer();
    else
        m_msMIDIState = MIDI_PLAYING;
}
